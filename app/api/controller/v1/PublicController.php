...
use app\services\gxhc\IndustryCategoriesServices;
...
class PublicController
{
        ...
        try {
            validate([
                'pdf_file' => [
                    ...
                    'fileExt'  => 'pdf',
                    'fileMime' => 'application/pdf' // 强制校验MIME类型
                ]
            ])->check(['pdf_file' => $file]);
            // 自定义文件名生成规则
            $filename = md5(uniqid()) . '.pdf';
            $savename = \think\facade\Filesystem::disk('public')
                ->putFileAs('bp' . '/' . date('Ymd'), $file, $filename);
            $dir = sys_config('site_url') . 'uploads/' . $savename;
            return app('json')->success('文件上传成功', ['name' => $filename, 'url' => $dir]);
    ...
    public function upload_file(Request $request)
    {
        ...
        try {
            ...
            $savename = \think\facade\Filesystem::disk('public')
                ->putFileAs('supplyFile' . '/' . date('Ymd'), $file, $filename);
            $dir = sys_config('site_url') . 'uploads/' . $savename;
            return app('json')->success('文件上传成功', ['name' => $filename, 'url' => $dir]);
        } catch (\think\exception\ValidateException $e) {
            return app('json')->fail($e->getMessage());
    ...
    {
        try {
            $file = public_path() . '/statics/miniprogram/data/colleges.json';
    ...
    {
        try {
            $file = public_path() . '/statics/miniprogram/data/enterprise.json';
            if (!file_exists($file)) throw new \Exception('文件不存在');
            $content = file_get_contents($file);
            $result = json_decode($content, true);
            if (json_last_error() !== JSON_ERROR_NONE) {
    ...
    public function get_industry_categories(Request $request)
    {
        ...
        return app('json')->success(app()->make(IndustryCategoriesServices::class)->getList($where));
    }

    // 查询一级下面的二三级分类
    public function get_industry_sub_categories(Request $request, IndustryCategoriesServices $services)
    {
        [$pid] = $request->getMore([
            ['pid', 0],
        ], true);
        if (!$pid) return app('json')->fail('参数错误');
        $list = $services->getSubList($pid);
