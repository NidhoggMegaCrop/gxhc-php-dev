...
use app\services\gxhc\UserIdentityServices;
...
class UserController
{
    ...
    public function user(Request $request)
    {
        ...
        $uid = (int)$request->uid();
        $user['auth'] = app()->make(UserAuthServices::class)->getOne(['uid' => $uid]);
    ...
    public function edit(Request $request)
    {
        ...
        if (!$data['avatar'] && $data['nickname'] == '' && $data['gender'] == '' && $data['email'] == '') {
        ...
        if ($this->services->eidtNickname($uid, $updateData)) {
            return app('json')->success(100014);
        }
        return app('json')->fail('请填写正确内容');
    }
    ...
    {
        $uid = (int)$request->uid();
        $userIdentityServices = app()->make(UserIdentityServices::class);
        ...
        // 验证必要字段
        if (empty($data['certName']) || empty($data['certNo'])) {
            return app('json')->fail('姓名和身份证号不能为空');
        }
        ...
        // 检查是否已存在认证记录
        $exist = $userIdentityServices->getRealName($uid) ?? [];

        if ($exist) {
            // 如果已认证成功(status=2)，不允许修改
           if ($exist['status'] == 3) {
                // 添加新记录
                $result = $userIdentityServices->save($saveData);
            } else {
                // 更新记录（状态为等待审核(1)的情况）
                $result = $userIdentityServices->update(['uid' => $uid], $saveData);
            }
        } else {
            // 新增记录
            $result = $userIdentityServices->save($saveData);
        ...
        if ($result) {
            return app('json')->success('实名认证信息保存成功');
        } else {
    ...
    public function getRealName(Request $request)
    {
        ...
        // 查询该用户最新的实名认证信息
        $realNameInfo = $userIdentityServices->getRealName($uid) ?? [];

        return app('json')->success($realNameInfo);
    }
}
