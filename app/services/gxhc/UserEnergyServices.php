...
use app\dao\gxhc\UserEnergyDao;
 ...
 */
class UserEnergyServices extends BaseServices
{
    ...
    protected $incomeData = [
        ...
        'deduction' => [
            ...
            'category' => 'integral',
            'type' => 'deduction',
            'mark' => '升级BP-PLUS版使用{%number%}能量抵扣{%number%}元',
            'status' => 1,
            'pm' => 0
        ],
        'pay_product' => [
     ...
     * @param UserEnergyDao $dao
     */
    public function __construct(UserEnergyDao $dao)
    {
        $this->dao = $dao;
    }
    ...
    public function pointRecord($where)
    {
        ...
        $status = [
            ...
            'system_sub' => '系统减少能量',
            'gain' => '下单赠送能量',
            'product_gain' => '购买商品赠送能量',
            'deduction' => '下单能量抵扣',
            'lottery_use' => '参与抽奖使用能量',
            'lottery_add' => '抽奖中奖赠送能量',
            'order_deduction' => '扣除订单下单赠送能量',
            'storeIntegral_use' => '能量兑换商品',
            'pay_product_integral_back' => '返还下单使用能量',
            'mint_redemption_code' => '生成兑换码',
            'redeem_code' => '兑换能量',
            'sign' => '签到获得能量',
        ];
        ...
        //关联订单
        /** @var BpOrderServices $orderServices */
        $orderServices = app()->make(BpOrderServices::class);
     ...
     * @return float
     */
    public function getUserEnergyBrokerageSum(int $uid, array $type = ['brokerage', 'brokerage_user'], $time = '')
    {
    ...
    public function getIntegralList(int $uid = 0, array $where_time = [], string $field = '*')
    {
        ...
        foreach ($list as &$item) {
            ...
            $item['userInfo'] = [];
            $userInfo = $userService->get($item['uid'],['avatar', 'nickname','phone']);
            if ($userInfo) {
                $item['userInfo'] = $userInfo->toArray();
                if (empty($item['userInfo']['avatar'])) {
                    $item['userInfo']['avatar'] = sys_config('h5_avatar');
                }
            }
        }
        $count = $this->dao->count($where);
    ...
    {
        $brokerageList = $this->dao->getUserEnergyList([
            'category' => 'now_money',
            'type' => 'brokerage',
            'link_id' => $id,
            'pm' => 1
        ...
        if ($UserEnergyData) {
            $res = $res && $this->dao->saveAll($UserEnergyData);
        }
     ...
     * @return array
     */
    public function getUserEnergyList(int $uid, int $type)
    {
        $where = [];
        $where['uid'] = $uid;
        $where['category'] = 'now_money';
        ...
        [$page, $limit] = $this->getPageValue();
        $list = $this->dao->getUserEnergyListByGroup($where, $field, 'time', $page, $limit);
        $data = [];
        if ($list) {
