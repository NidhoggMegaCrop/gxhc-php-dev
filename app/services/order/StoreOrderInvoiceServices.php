...
class StoreOrderInvoiceServices extends BaseServices
{
    ...
    public function getList(array $where)
    {
        ...
        $where['is_pay'] = 1;
        $where['is_del'] = 0;
        $list = $this->dao->getList($where, $field, [], 'add_time desc', $page, $list);
        $bpOrderServices = app()->make(BpOrderServices::class);
        foreach ($list as &$item) {
            $orderInfo = $bpOrderServices->getOne(['id' => $item['order_id']]);
    ...
    public function getOrderInvoiceList(array $where)
    {
        ...
        $where['is_pay'] = 1;
        $where['is_del'] = 0;
        $list = $this->dao->getList($where, '*', [], 'add_time desc', $page, $list); // 移除了原有的 ['order'] 关联

        /** @var BpOrderServices $bpOrderServices */
        $bpOrderServices = app()->make(BpOrderServices::class);

        foreach ($list as &$item) {
            // 使用 BpOrderServices 处理订单数据
            if (isset($item['order_id']) && $item['order_id']) {
                // 获取订单信息并整理
                $orderInfo = $bpOrderServices->getOne(['id' => $item['order_id']]);
                $item['order'] = $orderInfo;
                // if ($orderInfo) {
                //     $item['order'] = $bpOrderServices->tidyOrder($orderInfo, true);
                //     // 处理时间格式
    ...
    public function makeUp(int $uid, $order_id, int $invoice_id)
    {
        ...
        /** @var BpOrderServices $BpOrderServices */
        $storeOrderServices = app()->make(BpOrderServices::class);
