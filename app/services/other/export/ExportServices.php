...
{
    public function exportBpList($where)
    {
        /** @var BpRecordServices $BpRecordServices */
        $bpRecordServices = app()->make(BpRecordServices::class);
        $userServices = app()->make(UserServices::class);
        $data = $bpRecordServices->get_record_list($where)['list'];
        $header = ['ID', '昵称', '电话', '推荐人', '推荐人电话', '流水线编号', '类型', '状态', '进度', '原始文件名', '创建时间', '订单号', '实付金额'];
        $filename = 'BP列表_' . date('YmdHis', time());
        $export = $fileKey = [];
        if (!empty($data)) {
            $i = 0;
            foreach ($data as $item) {
                $userInfo = $userServices->getOne(['uid' => $item['uid']]);
                $ivtInfo = [];
                if (!empty($userInfo['spread_uid'])) {
                    $ivtInfo = $userServices->getOne(['uid' => $userInfo['spread_uid']]);
                }
                // 根据JavaScript逻辑转换的状态判断
                $upperStatus = strtoupper($item['status'] ?? '');
                $progress = $item['progress'] ?? 0;

                if (in_array($upperStatus, ['QUEUED', 'RUNNING']) && $progress < 100) {
                    $status = '诊断中';
                } else if (in_array($upperStatus, ['FAILED', 'CANCELED'])) {
                    $status = '诊断失败';
                } else if (in_array($upperStatus, ['SUCCEEDED', 'SUCCESS'])) {
                    $status = '诊断成功';
                ...
                $one_data = [
                    ...
                    'phone' => $item['phone'],
                    'ivt_nickname' => !empty($ivtInfo['nickname']) ? $ivtInfo['nickname'] : '',
                    'ivt_phone' => !empty($ivtInfo['phone']) ? $ivtInfo['phone'] : '',
                    'run_id' => $item['run_id'],
                    'target' => $item['target'] == 'export_optimization'  ? 'Plus' : 'Base',
                    'status' => $status, // 使用处理后的状态值
                    'progress' => $item['progress'],
                    'origina_filename' => $item['origina_filename'],
