...
每隔30秒执行自动更新BP状态
     ...
     */
    public $markList = [
        'bpStatusUpdate' => 'BP状态更新',
        'bpOrderCancel' => '未支付自动取消订单',
    ...
    {
        try {
            app()->make(BpRecordServices::class)->bpStatusUpdate();
            $this->crontabLog(' 执行BP状态更新');
        } catch (\Throwable $e) {
            $this->crontabLog('自动取消订单失败,失败原因:' . $e->getMessage());
        }
    }

    public function bpOrderCancel()
    {
        try {
            app()->make(BpOrderServices::class)->orderUnpaidCancel();
