...
class UserServices extends BaseServices
{
    ...
    public function getInviteCount(int $uid)
    {
        return $this->dao->count(['spread_uid' => $uid]);
    }
    ...
    public function index(array $where)
    {
        ...
        if ($list) {
            ...
            $userAuthServices = app()->make(UserAuthServices::class);
            ...
            foreach ($list as &$item) {
                ...
                $spredadUids = $shareRecordServices->getInviteColumnByUid($item['uid'], 1);
                $shareIds = $shareRecordServices->getInviteColumnByUid($item['uid'], 0);
                $item['spredadCount'] = !empty($spredadUids) ? count($spredadUids) : 0;
                $item['shareCount'] = !empty($shareIds) ? count($shareIds) : 0;

                $authInfo = $userAuthServices->getOne(['uid' => $item['uid']], '*');
    ...
    public function updateInfo(int $id, array $data)
    {
        ...
        if ($data['energy_status'] && $data['energy']) {//能量增加或者减少
            /** @var UserEnergyServices $userBill */
            $userBill = app()->make(UserEnergyServices::class);
            $energy_data = ['link_id' => $data['adminId'] ?? 0, 'number' => $data['energy']];
            if ($data['energy_status'] == 1) { //增加
                $edit['energy'] = bcadd($user['energy'], $data['energy'], 2);
                $energy_data['balance'] = $edit['energy'];
                $energy_data['title'] = '系统增加能量';
                $energy_data['mark'] = '系统增加了' . floatval($data['energy']) . '能量';
                $res3 = $userBill->incomeIntegral($user['uid'], 'system_add', $energy_data);
            } else if ($data['energy_status'] == 2) { //减少
                $edit['energy'] = bcsub($user['energy'], $data['energy'], 2);
                $energy_data['balance'] = $edit['energy'];
                $energy_data['title'] = '系统减少能量';
                $energy_data['mark'] = '系统扣除了' . floatval($data['energy']) . '能量';
                $res3 = $userBill->expendIntegral($user['uid'], 'system_sub', $energy_data);
            }
        } else {
            $res3 = true;
        }
    ...
    public function editOther($id)
    {
        ...
        $f[] = Form::radio('energy_status', '修改能量', 1)->options([['value' => 1, 'label' => '增加'], ['value' => 2, 'label' => '减少']]);
        $f[] = Form::number('energy', '能量', 0)->min(0)->precision(0)->max(999999);
